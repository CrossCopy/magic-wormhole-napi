...
async fn run() -> napi::Result<()> {
  ...
  match app.command {
    ...
    WormholeCommand::Completion { shell } => {
      ...
      //       let out =
      //         String::from_utf8(out).expect("Internal error: shell completion not UTF-8 encoded");
      //       let out = format!(
...
fn sender_print_code(
  ...
  let qr = qr2term::generate_qr_string(&uri).context("Failed to generate QR code for send link")?;
  writeln!(term, "{}", qr)?;
